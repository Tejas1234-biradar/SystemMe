...
const colors = ["#aab7ff", "#89a2ff", "#6b5fd3", "#ccd9ff"]; // Night sky colors
const emberColors = ["#ff9966", "#ff5e62", "#ffcc99"]; // Ember colors
...
export default function Home() {
  ...
  // Start audio when the component mounts
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Cleanup function to stop audio when component unmounts or audioStarted changes
    return () => {
      if (fireCrackle) {
        fireCrackle.pause();
        fireCrackle.currentTime = 0; // Reset to start
      }
      if (mysticalHum) {
        mysticalHum.pause();
        mysticalHum.currentTime = 0; // Reset to start
      }
    };
